...
<div class="container">
    <div class="event">
        <br>
        <div class="blog-add-button"><a href="newEvent" class="addblog">+ Create New Event </a></div>
        <br><br>
        <%--All Events show--%>
        <c:forEach items="${events}" var="event">
            <div class="eventbox">
                <a href="/EventsDescription?event=${event.id}">
                    <div>
                        <h2> ${event.title} </h2>
                        <span class="icon-map-marker">Date: ${event.date}</span><br>
                        <span class="icon-clock">Time: ${event.time}</span><br>
                        <span class="icon-pencil">Author: ${event.author}</span>
                    </div>
                </a><br>
                <a href="deleteEvent?id=${event.id}"><button type="submit" class="submit-input">Delete</button></a>
            </div>
        </c:forEach>
        <br>
    </div>
